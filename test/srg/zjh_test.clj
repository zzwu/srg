...
(facts "test handle ready event"
       (let [room (merge init-room {:seats {3 {:player-id "zzwu" :player-info {:bank 100}}}})
             ready-msg {:seat-no 3 :game-event :ready :time 1000}
             new-room (handle-game-event room ready-msg)]
         (get-in new-room [:seats 3 :state]) => :ready
         (get-in new-room [:ready-time]) => 1000))
...
(facts "test handle start event"
       (let [room init-room
             messages [{:player-id "zzwu" :player-info {:bank 100} :seat-no 0 :game-event :join-room}
                       ...
                       {:seat-no 0 :game-event :ready :time 1000}
                       {:seat-no 1 :game-event :ready :time 1001}
                       {:seat-no 3 :game-event :ready :time 1002}
...
(facts "test pk-result handler"
       (let [messages [{:player-id "zzwu"  :game-event :join-room :player-info {:bank 100} :seat-no 0 :join-time 1000}
                       ...
                       {:seat-no 0 :game-event :ready :time 1000}
                       {:seat-no 1 :game-event :ready :time 1000}
         ...
         new-room => {:base 10, :dealer 0,
                      ...
                      :pot 110,
                      :seats {0 {:amount 70, :cards [{:rank 6, :suit :clubs} {:rank 8, :suit :clubs} {:rank 5, :suit :clubs}], :player-id "zzwu", :player-info {:bank 140}, :seat-no 0, :state :in :join-time 1000 :ready-time 1000},
                              1 {:amount 40, :cards [{:rank 3, :suit :clubs} {:rank 7, :suit :clubs} {:rank 10, :suit :clubs}], :player-id "cdf", :player-info {:bank 60}, :seat-no 1, :state :out :join-time 1000 :ready-time 1000}},
...
(facts "test game-over handler"
       (let [messages [{:player-id "zzwu"  :game-event :join-room :player-info {:bank 100} :seat-no 0}
             ...
             new-room (reduce handle-game-event init-room messages)]
         new-room => {:base 10, :dealer 0, :history-bids [], :last-bid 10, :ready-time Long/MAX_VALUE
...
(facts "start-game test"
       (let [actions [{:game-action :join-room :seat-no 0 :player-id "zzwu" :player-info {:bank 1000} :join-time 1000}
                      ...
                      {:game-action :ready :player-id "zzwu" :time 1000}
                      {:game-action :ready :player-id "ddy" :time 1000}
                      {:game-action :ready :player-id "cdf" :time 1000}
         ...
         (into {} room) => {:base 10,
                            ...
                            :seats {0 {:cards [{:rank 4, :suit :spades} {:rank 13, :suit :hearts} {:rank 10, :suit :diamonds}], :player-id "zzwu" :player-info {:bank 1000}, :seat-no 0, :state :in :join-time 1000 :ready-time 1000},
                                    1 {:cards [{:rank 4, :suit :clubs} {:rank 9, :suit :spades} {:rank 10, :suit :hearts}], :player-id "ddy" :player-info {:bank 1000}, :seat-no 1, :state :in :join-time 1000 :ready-time 1000},
                                    2 {:cards [{:rank 8, :suit :diamonds} {:rank 13, :suit :spades} {:rank 14, :suit :clubs}], :player-id "cdf" :player-info {:bank 1000}, :seat-no 2, :state :in :join-time 1000 :ready-time 1000}}}
         events => [{:game-event :join-room, :player-id "zzwu", :player-info {:bank 1000}, :seat-no 0 :join-time 1000}
                    ...
                    {:game-event :ready, :seat-no 0 :time 1000}
                    {:game-event :ready, :seat-no 1 :time 1000}
                    {:game-event :ready, :seat-no 2 :time 1000}
             ...
             (loop [as actions
               ...
               (if (seq as)
                 (let [curr-events (play-action r (first as))
                   ...
                   (recur (rest as) (into es curr-events) new-room))
                 [r es]))]
         (into {} room) => {:base 10, :dealer 1, :ready-time Long/MAX_VALUE
